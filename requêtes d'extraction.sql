...
select trunc (orchestra_dossier.date_creation) as journee ,
		...
		count (distinct (case when orchestra_Message_type.MESSAGE_TYPE_CATEGORY = 'SIGNATURE' then orchestra_dossier.id end) ) as nombre_de_Signature,
        count (distinct (case when orchestra_Message_type.MESSAGE_TYPE_CATEGORY = 'CI_REQUEST' then orchestra_dossier.id end) ) as nombre_de_CI,
        count (distinct (case when orchestra_Message_type.MESSAGE_TYPE_CATEGORY = 'CI_REQUEST' then orchestra_dossier.id end) ) as nombre_de_rejection,


    count (distinct orchestra_dossier.id ) -count (distinct (case when orchestra_Message_type.MESSAGE_TYPE_CATEGORY = 'SIGNATURE' then orchestra_dossier.id end) )
    - count (distinct (case when orchestra_Message_type.MESSAGE_TYPE_CATEGORY = 'CI_REQUEST' then orchestra_dossier.id end) ) as  dossier_en_cours,
...
avg(JBPM4_HIST_TASK.DURATION_/3600) as duration_traitement,
		...
		count (distinct orchestra_dossier.id ) as nombre_de_dossiers,
			count (distinct (case when orchestra_Message_type.MESSAGE_TYPE_CATEGORY = 'SIGNATURE' then jbpm4_hist_task.dbid_ end) ) as nombre_de_Signature,
        count (distinct (case when orchestra_Message_type.MESSAGE_TYPE_CATEGORY = 'CI_REQUEST' then jbpm4_hist_task.dbid_ end) ) as nombre_de_CI,
        count (distinct (case when orchestra_Message_type.MESSAGE_TYPE_CATEGORY = 'CI_REQUEST' then jbpm4_hist_task.dbid_ end) ) as nombre_de_rejection,
